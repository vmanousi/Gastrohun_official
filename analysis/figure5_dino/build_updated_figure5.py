...
OUTPUT_ROOT = REPO_ROOT / "image_classification" / "output" / "Complete_agreement_40"
...
df_prediction = pd.read_json(DATA_DIR / "ScenarioA_Predictions.json")
df_bootstrap = pd.read_json(DATA_DIR / "ScenarioA-B_bootstrap.json")
...
df_combined.to_csv(RESULTS_DIR / "updated_scenarioA_bootstrap_metrics.csv", index=False)
print(f"Saved raw bootstrap metrics to {RESULTS_DIR / 'updated_scenarioA_bootstrap_metrics.csv'}")
...
df_summary.to_csv(RESULTS_DIR / "updated_scenarioA_summary.csv", index=False)
...
plt.tight_layout()
plt.savefig(RESULTS_DIR / "updated_figure5.png", dpi=200)
print(f"Saved plot to {RESULTS_DIR / 'updated_figure5.png'}")
